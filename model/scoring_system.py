...
from scorers import compute_skill_overlap, compute_workload_score
    ...
    workers = [
        {
            ...
            "active_tasks": 1, "max_tasks": 5,
            "past_tasks": [
                {"description": "Implemented REST API with JWT auth in FastAPI"},
                {"description": "Designed PostgreSQL schema for e-commerce app"},
            ]
        },
        {
            "id": 2, "name": "Bob",
            "skills": ["Python", "PostgreSQL"],
            "active_tasks": 4, "max_tasks": 5,
            "past_tasks": [
                {"description": "Built data pipeline with Python and PostgreSQL"},
            ]
        },
        {
            "id": 3, "name": "Carol",
            "skills": ["Java", "Spring Boot"],
            "active_tasks": 0, "max_tasks": 5,
            "past_tasks": []  # new worker, cold start
        },
    ]

    results = rank_workers(new_task, workers)

    for r in results:
        print(f"\n{r['worker_name']} — Score: {r['final_score']}")
        b = r["breakdown"]
        print(f"Text similarity:  {b['text_similarity']} (most similar: \"{b['most_similar_task']}\")")
        print(f"Skill overlap:    {b['skill_overlap']} ({b['match_ratio']}) | missing: {b['missing_skills']}")
        print(f"Workload score:   {b['workload_score']} ({b['active_tasks']} active tasks)")